...
public class OpenLibraryApiClient : IApiClient<Book>
{
    ...
    public OpenLibraryApiClient(IHttpClientFactory httpClientFactory, IDistributedCache distributedCache, IOptions<OpenLibraryOptions> options)
    {
    ...
    public async Task<ApiClientData<Book>> GetData(DateRangeFilter dateRange)
    {
        ...
        {
            return new ApiClientData<Book>(_clientName)
            {
